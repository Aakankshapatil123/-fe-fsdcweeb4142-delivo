...
const RestaurantDetails = () => {
  ...
  useEffect(() => {
    const fetchRestaurantData = async () => {
      try {
        ...
        console.log("RESTAURANT DETAILS:", restaurantResponse);
        ...
        console.log("RESTAURANT MENU:", menuResponse);
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        ...
        console.log("RESTAURANT REVIEWS:", response);

        const reviewList = response.result || [];
  ...
  const filteredMenus = menus.filter((menu) => {
    ...
    if (selectedCategory === "Veg") {
      return String(menu.foodType || "").toLowerCase() === "veg";
    }

    // NON-VEG
    if (selectedCategory === "Non-Veg") {
      return String(menu.foodType || "").toLowerCase() === "non-veg";
    }
  ...
  const handleExtraChange = (extra) => {
    ...
    const exists = selectedExtras.some(
      (item) => item.name === extra.name && Number(item.price) === extraPrice,
    );
    ...
    if (exists) {
      setSelectedExtras((prev) =>
        prev.filter(
          (item) =>
            !(item.name === extra.name && Number(item.price) === extraPrice),
        ),
      );
    } else {
      setSelectedExtras((prev) => [
  ...
  const handleAddToCart = () => {
    ...
    const finalPrice = Number(selectedMenu.price || 0) + extrasTotal;

    const cartItemId = [
      selectedMenu._id,

      ...selectedExtras.map((extra) => `${extra.name}-${extra.price}`).sort(),
        ...
        <div className="rounded-2xl bg-white p-6 shadow-md md:p-8">
          <div className="flex flex-col justify-between gap-4 md:flex-row">
            ...
            <div className="flex items-center gap-3">
              ...
              {restaurant.totalReviews !== undefined && (
        ...
        <div className="mt-10">
          <div className="mb-6">
            ...
            <p className="mt-2 text-gray-600">
              Explore delicious food from {restaurant.name}
            </p>
          </div>
          ...
          {menus.length === 0 ? (
            ...
            <div className="rounded-2xl bg-white p-12 text-center shadow-md">
              ...
              <p className="mt-2 text-gray-500">Try another menu category.</p>
                  ...
                  {selectedMenu.extras.map((extra, index) => {
                    const isSelected = selectedExtras.some(
                      (item) =>
                        ...
                        Number(item.price) === Number(extra.price),
                    );

                    return (
                      <label
                        key={extra._id || `${extra.name}-${index}`}
